...
@click.option('--n-ensemble', default=1, type=int,
              help="Train K independent members per iteration and use their "
                   ...
                   "dropout (K=1, the default, keeps MC dropout). Requires "
...
def main(head, testing, n_iterations, n_candidates, n_select, mc_samples, epochs, dropout, n_datasets, n_samples, val_fraction, output_dir, generate_data, min_gen_fraction, max_gen_attempts, gen_workers, selection_strategy, entropy_blur, entropy_beta, entropy_pool_size, candidate_generation, proximity_sampling, tolerance_sampling, target, target_value, no_mcmc_eval, config_file, sweep_index, early_stopping, patience, warm_starting, eval_data_path, compute_full_metrics, y_transform, mcmc_data_dir, mcmc_max_samples, static_eval_size, data_dir, use_mcmc_loader, train_baseline, resume_from, n_additional_iterations, gpu_ids, seed, n_ensemble, acq_uncertainty, laplace_prior_precision, d_model, num_layers, dim_feedforward):
    ...
    for iteration in range(start_iteration, n_iterations + 1):
        ...
        if not train_baseline:
            ...
            # Extra ensemble members: member 0 is the model trained above, so its
            # checkpoint stays the one every downstream consumer expects. Each
            # extra member is reseeded and never warm-started, keeping the
            # initialisations independent, which is what makes the spread
            # epistemic rather than a local sensitivity to dropout masks.
        ...
        def _acq_uncertainty(cands, return_predictions=False):
            ...
            if acq_uncertainty == 'laplace':
                # Draw count must exceed the read-out's feature dimension or the
                # candidate covariance is needlessly rank-limited, reproducing
                # the very defect this replaces. This architecture reads out
                # directly on dim_feedforward, so d = dim_feedforward + 1: 257 at
                # the standard width and 401 for the transformer-matched one. The
                # 512 floor covers both, but a dim_feedforward above 511 would
                # silently rank-limit it again. The draws are matmuls on the
                # features, not forward passes, so asking for many is cheap.
                #
