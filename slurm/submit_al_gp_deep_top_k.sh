#!/bin/bash
# ==============================================================================
# Slurm job: GP active learning — DeepGP, top_k selection
#
# Setup: cp slurm/cluster.conf.template slurm/cluster.conf  (edit for your cluster)
#
# Submit from repo root:
#   source slurm/cluster.conf
#   sbatch --partition="${CLUSTER_PARTITION}" --gres="${CLUSTER_GPU_GRES_2}" slurm/submit_al_gp_deep_top_k.sh
# ==============================================================================
#SBATCH --job-name=al_gp_deep_top_k
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
"${PYTHON}" active_learning_gp.py \
    ...
    --output-dir "${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_deep_gp_top_k_output}" ${RESUME_ARGS} \
    --gpu-ids "${GPU_IDS}"
