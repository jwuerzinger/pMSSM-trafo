...
#
# Defaults mirror run_active_learning_dnn.sh (full production run).
#
...
#   sbatch --partition="${CLUSTER_PARTITION}" --gres="${CLUSTER_GPU_GRES_2}" slurm/submit_al_dnn.sh
#
...
#   AL_OUTPUT_DIR         Output directory               (default: active_learning_dnn_output_slurm)
...
#   sbatch slurm/submit_al_dnn.sh
#
#   # Without data generation (ML pipeline only):
#   sbatch --export=ALL,AL_GENERATE_DATA= slurm/submit_al_dnn.sh
#
#   # Custom output directory:
#   sbatch --export=ALL,AL_OUTPUT_DIR=my_run slurm/submit_al_dnn.sh
# ==============================================================================
#SBATCH --job-name=al_dnn
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "[params] AL_OUTPUT_DIR=${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_dnn_output_slurm_${TIMESTAMP}}"
...
"${PYTHON}" active_learning_dnn.py \
    ...
    --output-dir "${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_dnn_output_slurm_${TIMESTAMP}}" \
    --early-stopping \
