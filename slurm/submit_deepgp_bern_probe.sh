...
t, v = int(st["al_n_train"][0]), int(st["al_n_val"][0])
...
def run(tag, lr, iters, patience):
    ...
    m = create_gp_model("deep_gp", xtr, ytr, xva, yva, n_dim=19, num_samples=8,
                        ...
                        num_inducing_max=256, num_hidden_dims=10,
                        num_middle_dims=0)
...
run("A current (lr 1e-3, pat 100)",     1e-3, 10000, 100)
run("B lr 1e-2, pat 300",               1e-2, 10000, 300)
run("C lr 1e-2, NO early stop (2000)",  1e-2, 2000,  None)
run("D lr 3e-2, pat 300",               3e-2, 10000, 300)
PY
