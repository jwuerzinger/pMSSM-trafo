#!/bin/bash
# ==============================================================================
# Slurm job: DNN AL — top_k selection, n_select=20000
#
# Equivalent to run_active_learning_dnn_top_k_20000.sh for Slurm.
#
# Submit from repo root (partition/gres come from cluster.conf):
#   sbatch $(slurm/cluster_flags.sh 2gpu) slurm/submit_al_dnn_top_k_20k.sh
#
# Or override on the command line:
#   sbatch --partition="${CLUSTER_PARTITION}" --gres="${CLUSTER_GPU_GRES_2}" slurm/submit_al_dnn_top_k_20k.sh
# ==============================================================================
#SBATCH --job-name=al_dnn_top_k_20k
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
"${PYTHON}" active_learning_dnn.py \
    ...
    --n-select 20000 \
    --output-dir "${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_dnn_output_top_k_n_select_20k_${TIMESTAMP}}" ${RESUME_ARGS} \
    --gpu-ids "${GPU_IDS}"
