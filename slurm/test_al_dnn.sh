#!/bin/bash
# ==============================================================================
# Test job: DNN (MC Dropout) active learning smoke test (1 GPU)
#
# Submit from repo root (partition/gres come from cluster.conf):
#   sbatch --partition="${CLUSTER_PARTITION_DEV}" \
#          --gres="${CLUSTER_GPU_GRES_1}" slurm/test_al_dnn.sh
#
# Expected output: /ptmp/jwuerzin/output/test_al_dnn_output/ with 5 AL iterations
# ==============================================================================
#SBATCH --job-name=test_al_dnn
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
"${PYTHON}" active_learning_dnn.py \
    ...
    --output-dir "${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/test_al_dnn_output}" \
    --early-stopping \
    --patience 50 \
