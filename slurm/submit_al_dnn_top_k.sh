#!/bin/bash
# ==============================================================================
# Slurm job: DNN-based active learning (PMSSMFeedForward + MC Dropout)
#
# Defaults mirror run_active_learning_dnn.sh with top_k selection.
#
# Submit from repo root (partition/gres come from cluster.conf):
#   sbatch $(slurm/cluster_flags.sh 2gpu) slurm/submit_al_dnn_top_k.sh
#
# Or override on the command line:
#   sbatch --partition="${CLUSTER_PARTITION}" --gres="${CLUSTER_GPU_GRES_2}" slurm/submit_al_dnn_top_k.sh
#
...
#   sbatch slurm/submit_al_dnn_top_k.sh
#
#   # Without data generation (ML pipeline only):
#   sbatch --export=ALL,AL_GENERATE_DATA= slurm/submit_al_dnn_top_k.sh
#
#   # Custom output directory:
#   sbatch --export=ALL,AL_OUTPUT_DIR=my_run slurm/submit_al_dnn_top_k.sh
# ==============================================================================
#SBATCH --job-name=al_dnn_top_k
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "[params] AL_OUTPUT_DIR=${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_dnn_output_top_k_slurm_${TIMESTAMP}}"
...
"${PYTHON}" active_learning_dnn.py \
    ...
    --output-dir "${AL_OUTPUT_DIR:-/ptmp/jwuerzin/output/active_learning_dnn_output_top_k_slurm_${TIMESTAMP}}" \
    --early-stopping \
