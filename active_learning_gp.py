...
def plot_data_histograms_gp(X, Y, idx_train, idx_val, output_dir, model_name, iteration, logger):
    """
    Plot histograms of all input parameters and target for training and validation sets.

    Args:
        X: Full input tensor (N, 19) - raw physical units
        Y: Full target tensor (N, 1) or (N,) - raw physical units
...
def main(testing, n_iterations, n_candidates, n_select, n_datasets, n_samples,
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        plot_data_histograms_gp(X, Y, idx_train, idx_val, al_hist_dir, "AL", iteration, logger)
        ...
        plot_data_histograms_gp(X_baseline, Y_baseline, perm_base[:n_train_base], perm_base[n_train_base:],
